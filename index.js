...
class Contenedor{
    ...
    async save(objeto){
        try {
            const contenido = await promises.readFile("./Test.txt", "utf-8")
            ...
            await promises.writeFile("./Test.txt", arrayCompleto)
    ...
    async deleteById(numId){
        try {
            ...
            if (info.some(item=>item.id === numId)) {
                ...
                await promises.writeFile("./Test.txt", resultado)
    ...
    async deleteAll(){
        try {
            ...
            await promises.writeFile("./Test.txt", arrayVacio)
